...
public partial class CardData : SingletonD<CardData>
{
    ...
    public IEnumerator HA(Card card,List<TargetObject> tar)
    {
        ...
        M_TurnManager.instance.PlayIronDemonCommandVoice(tar[0], tar[1]); // 사운드 출력
    ...
    public IEnumerator H16(Card card, List<TargetObject> tar)
    {
        ...
        foreach(TargetObject player in M_TurnManager.instance.spawnedPlayerList)
        {
            ...
            {
                hasDebuff = true;
                int index = player.buffs.FindIndex(buff => buff.type == BuffType.BOONGGUI);
                player.buffs[index].value = 0;
                player.buffs.RemoveAt(index);
            }
            if(player.buffs.FindIndex(x => x.type == BuffType.SOIRAK) != -1)
            {
                hasDebuff = true;
                int index = player.buffs.FindIndex(buff => buff.type == BuffType.SOIRAK);
                player.buffs[index].value = 0;
                player.buffs.RemoveAt(index);
            }
            if(hasDebuff)
            {
                player.GainBuff(BuffType.ICHI_DEFENSE,2,false,false,false,false,tar[0],card);
            }
        } 
        yield return new WaitForSeconds(1.33f);
        M_DimmingManager.instance.StopDimming(M_TurnManager.instance.spawnedPlayerList);
    ...
    public void FuryOfIronEffect(TargetObject owner)
    {
        foreach(CardOnHand pCard in owner.player.GetComponent<GamePlayerDeck>().cardOnHands)
        {
            if(pCard.card.baseCard.cardNumber == "H0" || pCard.card.baseCard.cardNumber == "H1")
            {
                pCard.card.tempEnhanced = true;
    ...
    public IEnumerator H26(Card card, List<TargetObject> tar)
    {
        ...
        M_TurnManager.instance.StartAnimation(tar[0],0,"Buff0",false); // 단향이 공격 모션 
        yield return new WaitForSeconds(1f);
        tar[0].player.GetComponent<GamePlayerDeck>().usedCardName = "H26";
        tar[0].player.GetComponent<GamePlayerDeck>().maxRemoveCardCount = 3; // 제거용 카드 슬롯 3개로 설정
        yield return new WaitForSeconds(0.3f);
        M_DimmingManager.instance.StopDimming(tar.GetRange(0,1));
    }
    ...
    public IEnumerator H28(Card card, List<TargetObject> tar)
    {
        ...
        M_TurnManager.instance.StartAnimation(tar[0],0,"Buff0",false); // 단향이 공격 모션 
        yield return new WaitForSeconds(1f);
        int index = tar[0].GainBuff(BuffType.GROWTHSPURT, 0, false, true, false, false, tar[0],card);
    ...
    public IEnumerator H41(Card card, List<TargetObject> tar)
    {
        ...
        {
            index = tar[1].GainBuff(BuffType.FLOWERPOWDER,5,true,false,false,true,tar[0],card);
    ...
    public IEnumerator H49(Card card, List<TargetObject> tar)
    {
        ...
        M_EffectManager.instance.RpcEffectBackFallingLeaf(tar[0].transform.position, 98);
