...
public class CardOnHand : NetworkBehaviour
{
    ...
    {
        cardOnHand.GetComponent<CardOnHand>().isMoving = true;
        ...
        cardTransform
            ...
            .OnComplete(() => {
                cardOnHand.GetComponent<CardOnHand>().isMoving = false;
            });
    }
    ...
    public void CardOnHandThrowAwaySequence(CardOnHand cardOnHand)
    {
        ...
        sequence.Append(cardOnHand.transform.DOScale(new Vector3(0.5f, 1f, 0f), 0.5f));
        sequence.Join(cardOnHand.transform.DORotate(new Vector3(0f, 0f, -90f), 0.5f));
        sequence.Join(cardOnHand.transform.DOMove(new Vector3(0f, 0.5f, 0f), 0.5f).SetEase(Ease.OutSine));

        // 시퀀스에 사이즈 축소, 오른쪽으로 90도 회전, 현재위치에서 화면의 우측하단 방향으로 포물선 이동 애니매이션 추가
        sequence.Append(cardOnHand.transform.DOScale(new Vector3(0.5f, 1f, 0f), 0.5f));
        sequence.Join(cardOnHand.transform.DORotate(new Vector3(0f, 0f, -90f), 0.5f));
        sequence.Join(cardOnHand.transform.DOMove(Camera.main.ViewportToWorldPoint(new Vector3(1f, 0f, 0f)), 0.5f).SetEase(Ease.InOutCirc));
        sequence.OnComplete(() =>
        {
