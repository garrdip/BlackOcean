...
## 34회차 완료 작업 (G53 선택 강제 + 기획 확정 잔여분 정리 — 카드 감사 완결)

### ✅ 코드의 "수정 필요!!!" 주석 해소 (기획 확정 2026-07-10)

- **검증 결과**: 핵심 흐름(2장 드로우 → 패에서 2장 선택 → 버린덱)은 설명대로 구현돼 있었음. 어긋남은 선택 강제 부재 — 0~1장만 선택하고 확정 가능해 "공짜 2장 드로우" 악용 소지.
- **수정**: `CardOnHandRemovePopUp` 확인 버튼에서 G53 사용 시 2장(패가 그보다 적으면 전부) 미만 선택이면 확정 차단. G53이 `usedCardName`을 설정해 분기 (H26 창조는 자유 선택 유지). 검증을 팝업 닫기 '전'으로 이동.

### ✅ 기획 확정 잔여분 일괄 반영

- **G6 계열 4장**: 데이터로만 유지, 실사용 없음 — 의도적 확정 (컨텐츠 작업 목록에서 제외)
- **HERO 보너스**: 영웅 카드당 3 (BalanceDB `HERO_BONUS_PER_CARD` 1→3, BuffDB 16행 설명의 N도 3으로 확정)
- **카드 감사(CARD_AUDIT.md) A~G절 전체 완결** — 21~34회차, 컨텐츠 작업 목록 소진

---

## 33회차 완료 작업 (E48·E51 "N배" 수치 확정·구현)

### ✅ 카드 감사 마지막 수치 플레이스홀더 해소 (기획 확정 2026-07-10)

- **E48 그랜디오소**: 버린덱으로 보낸 2장 중 E48 1장 포함 → 피해 2배, 2장 모두 → 3배. 기본 8은 카드 실행 시, 추가분(8×개수)은 선택 완료 콜백(`E48_SelectionCallBack`)에서 — 선택 팝업이 카드 실행 후 열리는 구조라 대상을 `grandiosoTarget`에 보관 후 콜백에서 사용 (대상 사망 시 스킵). 추가 피해는 파괴의권능 배수 미적용(실행 컨텍스트 종료 후).
- **E51 아프나요?**: 파괴의권능 피해 배수 2배 — `ApplyPowerOfDestruction`에서 발동 시(배수>1) ×2. 반피 4배·광기 6배·반피+단말마 6배. 체력 소모는 2 유지.
- CSV 설명 확정: E48 "[보낸 카드 중 E48이 있으면 피해가 2배 두 장 모두 E48이면 3배]", E51 "파괴의권능 피해 배수를 2배로 받습니다".

---

## 32회차 완료 작업 (F절 의심 9건 — 건별 기획 확정·해결)

### ✅ 카드 감사 F절 완결 (유저와 건별 문답으로 전건 확정, 2026-07-10)

1. **G26 쐐기박기**: 시전자가 부여한 압도만 계산·제거 (종전엔 피해는 전원 합산·제거는 첫 항목 1개 — 불일치까지 해소). CSV "자신이 부여한 압도"로 수정
2. **기사도 3종 (G55·G64·G65)**: G30 반복·G38 힘의이치 트리거 적용 — G55 패 복귀 1회 + 이치는 발동 횟수만큼(반복 시 2), G64 발동 횟수만큼 드로우, G65 발동 횟수만큼 덱 생성. G64·G65의 이치 연계도 동일 규칙 적용
3. **E7 돌로레**: 고정 체력 손실 — 신규 공통 메서드 `TargetObject.LosePlayerHP` (방어·단말마·붕괴 증폭 무시, 광기 변신·템페스토소 누적은 유지). "체력을 잃습니다" 계열 재사용 가능
4. **E38 자비의 권능**: N = 잊혀진덱으로 보낸 E10 개수 — 장당 개수만큼 회복(총 개수²). CSV 문구 확정
5. **E39 알레그레토**: 붕괴 부여 '전' 보유 판정 — 이미 붕괴인 적에게만 추가 피해 10 (콤보 카드화)
6. **E23 삼색빛별**: 실제 준 피해만큼 방어 — cardDamageDealt 델타 (도돌이표 반복에도 실행분만 정확)
7. **H5/H6 메마른샘**: 현행 유지 확정 (maxIchi 직접 증가 — 의도된 구현)
8. **H46 꽃가루 폭발**: 즉시 2배는 기구현 확인, 변환 훅만 턴시작 → 턴종료로 통일 (이중 발동 소지 제거)
9. **H57 철에 피는 꽃**: 철귀 위치가 몬스터면 isDebuff=true 분기 — 타 꽃가루 카드 관례와 통일

---

## 31회차 완료 작업 (시각 이펙트 2건 + H0_E 조건부 화합 구현)

### ✅ G절 TODO 코드 작업분 완결

- **E17 얼마나 버틸까요 / E18 아니마토**: 방어 제거 시 `RpcEffectEnergyExplosion` 연출 추가 (E17은 적 위치, E18은 자신 위치 — 캐릭터 중립 이펙트 재사용).
- **H0_E 강화 철의 손톱**: TODO 주석은 "화합 이펙트"였지만 실제로는 **조건부 화합 효과 자체가 미구현**이었음 — CSV "대상에 철귀가 있을 시 화합 발동"은 특성이 아닌 조건부라 파이프라인 자동 발동이 없음. 사용 시점에 철귀가 대상 위에 있으면 H0 실행 후 `HWAHAP()` 직접 호출 (대상 사망 시 스킵).
- **낡은 TODO 주석 정리**: ErisAnimation 변신 분기(기구현)·E19 붕괴 플래그(관례 일치 확인)·G56(30회차 구현) 주석 현행화.
- **잔여 TODO 전황**: 코드 작업 가능분 소진. 남은 것은 전부 기획 확정 대기 — E48·E51("N배" 수치), G53("수정 필요" 의도 불명), F절 의심 9건, G6 계열 스텁.
...
- **기획 확정 사항**: ① 고행 3장이 **손패에** 전부 있어야 변신 (써버리면 그 전투는 불가 — 드로우는 종류당 1회) ② 3번째 고행 드로우 시 자동 변신 ③ 지속 3턴 (추후 변경 가능 → BalanceDB `HERO_DURATION_TURN`) ④ 이치 1 소비는 고행 드로우에만.
- **고행 드로우 (`DrawGoHengCard`)**: 이치 1 소비 추가 (부족 시 발동 안 함, 턴당 1회 소진도 안 됨). 드로우 후 고행 I·II·III(_E 포함)이 전부 손패에 있으면 `GeorkTransform()` 자동 발동.
- **변신 (`GeorkTransform`)**: `isTransformed = true` (기존 소비처 G61 신의 뜻 60딜·G62 구세주 전체 방어·"H" 애니 프리픽스가 즉시 살아남), HERO 버프 부여(값 = 잔여 턴), 턴 시작/종료 훅 등록, 이치의저주 → 영웅 카드 변환, Transform → HIdle 애니.
- **카드 변환 (`ConvertCurseHeroCards`)**: GEORK CURSE(G0~G7 계열) ↔ `Gn_H` — `_E`는 `Gn_H_E`로. baseCard만 교체한 복사본이라 경험치·강화 상태 보존. 전투 덱(패·뽑을덱·버린덱)만 대상. CSV 매핑 무결성 검증 완료(16종 전부 짝 존재), 영웅 카드 효과 메서드 8종 기존재 확인.
- **카드 뷰 갱신**: `CardOnHand.OnChangeCardData`에서 카드 번호가 바뀐 경우 `InitCardTemplateByCharacter` 재실행 — 변환 시 이름·설명뿐 아니라 **일러스트·틀·문장(영웅 템플릿)**까지 교체, 해제 시 원복. 영웅 일러 8종(G0_H~G7_H)이 CardIllustAtlas에 전부 존재함을 확인. 재초기화 대비 `InitCardExpBar`에 기존 블록 클리어 추가(중복 생성 방지).
- **카드 전환 연출 (`CardConvertFlash`)**: 유효한 카드 → 다른 카드 전환 시 풀 화이트 페이드인(0.3s) → 흰 상태에서 교체 → 페이드아웃(0.8s)으로 원래 색 복귀. 캔버스 최상단 흰 오버레이(런타임 생성·재사용, raycast 차단 없음). 초기 스폰·일상 갱신(비용/경험치)은 연출 없이 즉시 반영.
- **턴 시작 보너스 (`HERO_TurnBeginEffect`)**: 소유(패+뽑을덱+버린덱) 영웅 카드 수 × N 만큼 힘의이치·방어 획득. N = BalanceDB `HERO_BONUS_PER_CARD`(기본 1 — **BuffDB 16행의 플레이스홀더 N, CSV에서 튜닝**). 힘의이치는 턴 종료 훅에서 회수("그 턴에만") — 다른 경로의 힘의이치는 건드리지 않음.
- **지속/해제**: 턴 종료마다 HERO 버프 -1 (M_TurnManager.PlayerEndTurnEffect), 0이 되면 `RevertGeorkTransform()` — 카드 원복·Idle 복귀. 변신한 턴 포함 3턴 유지. TargetObject가 전투마다 재생성되므로 전투 간 누수 없음.
- **플레이 테스트 피드백 반영**: ① 변신 시 고행 II/III 디버프(GOHANG2/3_DEBUFF) 제거 — 원래 저주 고행 '사용' 시 제거되는데 변신 후 카드가 _H가 되어 제거 경로가 소실되던 버그. 해제 시 재부여 없음(사용 해소와 동일 취급). ② 영웅판 고행 이름이 저주판과 동일하던 CSV 미작성 채움 — G0_H~G2_H "찬란한 고행 I/II/III" (**임시 이름 — 기획 확정 필요**. G3~G7 영웅판 네이밍 패턴 준용).
- 검증: 컴파일 0건 + 리플렉션(신규 메서드·필드 6종, BalanceDB 키 2종, CSV 매핑, 위빙 유지).

---

## 28회차 완료 작업 (파괴의권능 시스템 구현 — 에리스 패시브)

### ✅ 카드 감사 D절 마지막 항목 — BuffDB 정의를 스펙으로 시스템 신설

- **구현 위치**: `TargetObject.CharacterSpecific.cs`의 `ApplyPowerOfDestruction(Card)` + `destructionMultiplier` 필드. 창조의권능이 `GeneralGetDefense`에 캐릭터 체크로 구현된 것과 동일하게 **에리스 상시 패시브**로 해석 (버프 부여 카드가 어디에도 없음 → 부여형 아님).
- **체력 소모**: 공격 카드 사용 시(카드 큐 파이프라인, RunCard 직전) 체력 2 소모 — `Mathf.Max(1, ...)`로 1 미만 불가, 코스트라 방어/단말마 증폭 미적용, 템페스토소 누적에는 포함.
- **피해 배수**: 소모 후 체력 절반 이하 ×2, 광기(MAD) ×3 — BuffDB의 "??? 상태"는 광기로 해석(실플레이 유일 변신 상태), CSV 문구도 "광기 상태시"로 확정. 단말마(DEATHTHROES) 보유 시 발동 배수 +1 (24회차 보류분 해소). 적용은 `DamageToMonster` 최상단(붕괴 ×1.5보다 먼저 → 복리), 도돌이표 반복 실행까지 유효 후 리셋.
- **제외**: E10/E10_E(뒤틀리는 생명)는 CSV 명시대로 체력 소모·배수 모두 미적용.
- **잔여**: E51 "N+1배 더"·E48 "N배"는 N 수치 미확정 — 기획 확정 대기. 손패 피해 미리보기에 권능 배수 반영은 포매터가 카드 컨텍스트를 몰라 보류(원하면 후속 설계).
- 검증: 컴파일 0건 + 리플렉션(필드·메서드 존재, BuffDB 행 콤마 무결성 — 필드 수 3=헤더와 동일).

---

## 26회차 완료 작업 (에리스 TODO 카드 4장 구현 — E15·E46·E54·E57)

### ✅ G절 TODO 중 CSV 스펙이 확정된 4장 구현 + 공격 카드 카운터 인프라 신설

- **카운터 인프라 (GamePlayerDeck)**: `numOfUsedAttackCardOnTurn`(턴 단위 — PlayerPreEffect·전투 종료 시 리셋), `numOfUsedAttackCardOnBattle`(전투 단위), `e46DamageBonus`(전투 단위 — 기획 확인: 게임 지속이면 밸런스 붕괴). 셋 다 전투 종료 시 리셋. 카드 큐 파이프라인에서 공격 카드 실행 '후' 증가 — 실행 중인 카드 자신은 카운트 미포함. 기존 카운터(numOfUsedCard 등)와 동일하게 세이브 미저장.
- **E15 권능:파괴**: 피해 3 + 이번 전투 동안 사용한 공격 카드 수 × 2 (기획 확인으로 게임 지속 → 전투 지속으로 교정, CSV 설명 문구도 "이번 전투동안"으로 수정)
- **E46 헤일로**: 사용마다 `e46DamageBonus` +1 — 이 이름의 카드 전체 피해가 1씩 누적 증가 (전투 지속 — 덱 복사본은 게임 지속이나 피해 누적은 전투 종료 시 리셋)
- **E54 산개 성단**: 기본 1타 + 이번 턴에 쓴 공격 카드 수만큼 반복 타격 (대상 사망 시 중단)
- **E57 거친 파도처럼**: `GetTotalCostOfCardOnHand`에서 소유(deck) 은하수 카드당 비용 1 감소 (자신 포함, 고행 III 고정 비용이 우선)
- 검증: 컴파일 0건 + 리플렉션(신규 필드 3종·카드 메서드 4종 존재, GamePlayerDeck/M_TurnManager 위빙 유지).
- **보류 사유 기록**: E48·E51은 CSV 설명의 "N배"가 플레이스홀더(수치 미확정)라 기획 확인 필요. E19는 기존 BOONGGUI 부여 관례(E16)와 플래그가 이미 일치.

---

## 25회차 완료 작업 (에리스 버프 시스템 2차 — 별무리·도돌이표, 완결)

### ✅ 카드 실행 파이프라인 개입형 버프 2종 구현 — 무효과 버프 9종 전부 해소

- **REPEATMARK 도돌이표 (E24)**: 카드 큐 파이프라인에서 공격 카드 실행 직후, 시전자가 도돌이표 보유 시 `RunCard` 1회 추가 실행 (첫 실행으로 대상 사망 시 중단). "이번 턴" — isDecrease 감쇠로 교정.
- **BYEOLMURI 별무리 (E5/E6/E11/E23/E36/E45/E47/E58)**: 사용 '전' 보유 기준으로 판정(자기 자신이 부여하는 스택은 다음 카드부터) 후 스택 1 소모.
  - 공격 카드 = 준 피해 절반 방어: 신규 `cardDamageDealt` 누적(카드 실행 전 리셋, DamageToMonster에서 시전자별 누적 — 붕괴/개화 반영 후 수치) ÷ 2 방어 획득
  - 전략 카드 = 사용 후 다시 패: 소각 대신 `ReturnToCardOnHand` (큐 표시는 사용된 것으로 유지)
  - 그 외 카드 = 비용 2 감소: `GetTotalCostOfCardOnHand`에서 할인 (고행 III의 "비용 1 고정" 오버라이드보다 앞 순서 — 고정이 우선)
- 검증: 컴파일 0건 + 리플렉션(M_TurnManager 위빙 14개 유지, 신규 필드 존재).
- **플레이 검증 포인트**: E24 후 공격 카드 2회 타격(같은 턴만), 별무리 보유 중 ① 공격 카드 → 피해 절반 방어 ② 전략 카드 → 패로 복귀 ③ 축복/저주 카드 비용 -2 표시·지불 ④ 사용마다 스택 1씩 감소·0에서 아이콘 소멸, E5(부여 카드 자신은 강화 미적용) 확인.

**에리스 버프 시스템 완결**: 24·25회차로 무효과 버프 9종 전부 구현. 잔여는 파괴의권능 시스템(신규 설계 필요 — E10/E50/E51 관련 문구)뿐.

---

## 24회차 완료 작업 (에리스 버프 시스템 1차 — 트리거형 7종 구현)

### ✅ 부여만 되고 소비 코드가 없던 에리스 버프 중 7종에 실제 발동 로직 구현

| 버프 | 카드 | 구현 지점 | 동작 |
|---|---|---|---|
| DICHOTOMY 이분법 | E59 | `buffTrunBeginEffect` 훅 등록 | 턴 시작 시 체력 2 소모(StaticDamage, 고행 관례) + E10 1장 패 생성 |
| SIGNOFEND 종말의 징조 | E29 | 뽑을덱→버린덱 콜백 말미 | 이동마다 적 전체 피해 2 |
| TEMPESTOSO 템페스토소 | E20 | DamageToPlayer/Static 양쪽 | 잃은 체력 누적 10마다 1드로우 (실losing분만 집계 — 방어 흡수 제외) |
| ECLIPSE 월식 | E21 | DamageToMonster | 시전자가 넣은 피해(붕괴·개화 반영 후)만큼 방어 획득. "이번 턴"은 isDecrease 턴 감쇠로 |
| ENHANCESKIN 허물 강화 | E35 | 신규 `TargetObject.HealPlayer()` | 이번 턴 회복이 방어로 전환. 전투 내 회복 4경로(E10/E18/E38/에리스 방어→체력 전환) 공통 경로화 |
| ENDOFDISTORTION 뒤틀림의 끝 | E33 | GeneralGetDefense 에리스 분기 | 만피 상태에서 자신의 카드로 얻을 방어를 적 전체 피해로 전환 |
| DEATHTHROES 단말마 | E50 | DamageToPlayer | 받는 일반 피해 2배 (고정피해는 미증폭 — 고정의 의미 유지). 파괴의권능 +1배는 시스템 부재로 TODO 유지 |

- **부여부 결함 추가 수정**: E20·E21은 value 0 + 비영구 플래그라 `GainBuff`의 `value==0 && !isInfinity` 조기 반환에 걸려 **버프 등록 자체가 안 되고 있었음** — 지속 구분에 맞는 플래그(전투 지속 isInfinity / 이번 턴 isDecrease)로 7종 전부 교정.
- **설계 판단 기록**: ① "이번 게임동안"(E33)은 버프가 전투 단위 TargetObject에 실리므로 현 구조상 전투 지속으로 구현(기존 '이번 게임' 계열 카드와 동일한 한계) ② 이분법 체력 소모는 고행(G3) 관례대로 StaticDamage(방어 우선 소모) ③ 에리스의 방어→체력 전환 회복도 HealPlayer 경유라 허물 강화와 상호작용(방어 유지) — E35의 존재 이유로 판단.
- 검증: 컴파일 0건 + 리플렉션(HealPlayer/누적기/턴훅 존재, 위빙 유지) + 버프 7종 소비처 grep 확인.
- **플레이 검증 포인트**: E59 턴 시작 체력-2·E10 생성, E29 후 드로우 시 적 전체 피해, E20 후 피격 10 누적 드로우, E21 후 공격 시 방어 획득·다음 턴 소멸, E35 후 회복이 방어로, E33 만피에서 방어 카드 → 적 전체 피해, E50 후 받는 피해 2배.
- **잔여(25회차 예정)**: BYEOLMURI 별무리(다음 카드 강화 3분기) + REPEATMARK 도돌이표(공격 카드 2회 실행) — 카드 실행 파이프라인 개입이 필요해 별도 회차.

---

## 23회차 완료 작업 (카드 감사 후속 3차 — D절 로직 버그 7건)

### ✅ CARD_AUDIT.md D절의 수정 가능 로직 버그 전건 처리

| 카드 | 수정 |
|---|---|
| G2 고행 III | 비용 +1 디버프에서 G2/G2_E 자신 제외 (GamePlayerDeck.GetTotalCostOfCardOnHand) |
| G16 성흔의 고통 | 누락됐던 패 생성 1장 추가 (뽑을덱 1장 + 패 1장) |
| G19 고통의 메아리 | ① 저주(CURSE) 카드 사용 시에만 발동하도록 필터 추가 (종전엔 모든 카드마다 발동) ② 일반 피해 → `StaticDamageToMonster`(설명의 "고정피해") |
| G31 고통 전가 | 저주 카드 집계를 패+뽑을덱+버린덱 → **패 한정**으로 (설명: "내 패에 있는") |
| H32 포식자 | 처치 여부와 무관하던 현재 전투 크기+1을 처치 분기 안으로 이동 |
| E10 뒤틀리는 생명 | 초과 회복 분기의 no-op(`playerMaxHP = playerMaxHP`) → `playerHP = playerMaxHP` — HP max-1/max-2에서 회복분 통째 소실되던 버그 해소 |
| E60 찢어줄게요 | "체력 절반 이하 시 2회" 조건을 도달 불가능한 `ErisMode.ANGER` 대신 실제 HP 검사(`playerHP <= playerMaxHP/2`)로. MAD(변신) 3회 유지 |

- 검증: 컴파일 0건 + 감사 스크립트 재실행 (바인딩 398 유지, 잔여 플래그 G6 스텁 4건뿐).
- **플레이 검증 포인트**: G2 사용 후 G2 자신 비용, G16 생성 2장 위치, G19가 저주 카드에만 반응, G31 피해량(패 저주 수×8), H32 미처치 시 크기 불변, E10 회복 경계값(만피-1), E60 반피 이하 2회.
- **감사 잔여**: F절 의심 9건(의도 확인 필요 — 별도 논의), 에리스 버프 시스템 구현 + G/TODO(컨텐츠 설계 작업).
