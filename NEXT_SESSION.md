...
- **P1-4a(PlayerRegistry)**: 커밋됨(419a7b8c), 플레이 검증 완료 판정(13회차 커밋 승인으로 간주).
- **13회차 P2-16(죽은 코드 정리)**: 커밋·푸시 완료(44b3e219).
- **14회차 P2-15a(PlayerRegistry.Local 캐싱 79곳)**: 작업 트리에 **미커밋 상태**. 컴파일 0건 + 리플렉션 검증 완료. **플레이 테스트 통과 후 커밋할 것.**
...
14회차(P2-15a) 변경 확인 포인트 — 로컬 플레이어 조회가 캐시로 바뀌어 **로컬 UI 전반**이 대상:

- [ ] 게임씬 진입 직후: 덱 3종 버튼 팝업, 턴 종료 버튼, 카드 사용/타겟팅
- [ ] 맵: 방 클릭·투표, 투표 레이아웃 표시
- [ ] 맵↔전투 왕복 후에도 위 항목 정상 (Local 캐시 수명 검증)
- [ ] NPC(소피아/류진솔/그림자/메르쿠리우스) 상호작용, 보스전 UI

**테스트 통과 시**: 14회차분 커밋 (메시지 예: "로컬 PlayerInterface 조회 79곳 PlayerRegistry.Local 캐시로 전환 (P2-15 1회차)") → 푸시
...
| 2 | **P2-15 매 프레임 폴링/캐싱 정리** | 🔶 P2-15a 완료 (14회차) | 잔여: M_SoundManager OnUpdate 믹서 폴링 이벤트화, LobbyPlayer(49회)·HexagonMapRoom(26회) GetComponent 캐싱 |
