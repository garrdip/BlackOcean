...
- **13~16회차**: 커밋·푸시 완료 (P2-16: 44b3e219 / P2-15a: 38e8cc1d / P2-15b: 940b96e6 / P2-15c: 0cd7f453).
- **17회차 P2-12(밸런스 데이터화 — BalanceDB.csv)**: 작업 트리에 **미커밋 상태**. 컴파일 0건 + 12개 키 전수 런타임 조회 검증 완료. **플레이 테스트 통과 후 커밋할 것.**
...
17회차(P2-12) 변경 확인 포인트 — 밸런스 값이 BalanceDB.csv 조회로 바뀜 (값은 동일해야 함):

- [ ] 게임 시작: 초기 HP 50 / 골드 100 / 이치 3(에리스 2) 표시
- [ ] 전초기지: 회복량 15
- [ ] 전투 승리: 골드 보상 10, 보상 카드 선택지 3장
- [ ] 카드상점: 가격 1, 매물 6장 / 패 제거 팝업: 3장 선택
- [ ] 콘솔에 `[BalanceData]` 에러 로그 없음

**테스트 통과 시**: 17회차분 커밋 (메시지 예: "밸런스 매직넘버 12종 BalanceDB.csv로 데이터화 (P2-12)") → 푸시
