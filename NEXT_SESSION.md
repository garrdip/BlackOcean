...
- **14회차 P2-15a(PlayerRegistry.Local 캐싱 79곳)**: 커밋·푸시 완료(38e8cc1d).
- **15회차 P2-15b(M_SoundManager 폴링 제거)**: 작업 트리에 **미커밋 상태**. 컴파일 0건 + 리플렉션 검증 완료. **플레이 테스트 통과 후 커밋할 것.**
...
15회차(P2-15b) 변경 확인 포인트 — 사운드 설정 적용 경로가 폴링 → 시작 시 명시 적용으로 바뀜:

- [ ] **게임 시작 시 저장된 볼륨/음소거 설정이 그대로 적용되는지** (핵심)
- [ ] 옵션에서 BGM/SFX/음성 슬라이더 실시간 변경, 음소거 토글
- [ ] 씬 전환(메뉴→룸→게임) 시 BGM 크로스페이드 정상

**테스트 통과 시**: 15회차분 커밋 (메시지 예: "M_SoundManager 매 프레임 볼륨 폴링 제거 (P2-15 2회차)") → 푸시
...
| 2 | **P2-15 매 프레임 폴링/캐싱 정리** | 🔶 a·b 완료 (14·15회차) | 잔여: LobbyPlayer(49회)·HexagonMapRoom(26회) GetComponent 반복 캐싱 |
