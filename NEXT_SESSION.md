...
## 카드 감사 후속 — 21~23회차 커밋 완료(cd1279e6, c0bfae26), 24회차 **미커밋**

24회차(에리스 버프 시스템 1차 — 트리거형 7종) 확인 포인트 (에리스 플레이 필요):

- [ ] E59 이분법: 다음 턴 시작마다 체력 2 감소 + '뒤틀리는 생명' 1장 패에 생성
- [ ] E29 종말의 징조: 이후 카드가 뽑을덱→버린덱 갈 때마다 적 전체 피해 2
- [ ] E20 템페스토소: 이후 잃은 체력 합계 10마다 카드 1장 드로우
- [ ] E21 월식: 이번 턴 공격으로 넣은 피해만큼 방어 획득, 다음 턴 버프 소멸
- [ ] E35 허물 강화: 이번 턴 체력 회복(에리스 방어→체력 전환 포함)이 방어로
- [ ] E33 뒤틀림의 끝: 만피 상태에서 방어 카드 사용 시 적 전체 피해로 전환
- [ ] E50 단말마: 이후 받는 일반 피해 2배
- [ ] 버프 아이콘 표시: 7종 부여 시 버프 인디케이터에 정상 표시되는지

**테스트 통과 시**: 24회차분 커밋 → 푸시. **문제 발견 시**: 콘솔 로그 확보 후 분석 요청.

**다음 예정**: 25회차 — 별무리(BYEOLMURI)·도돌이표(REPEATMARK): 카드 실행 파이프라인 개입형. 이후 F절 의심 9건 기획 확인.
